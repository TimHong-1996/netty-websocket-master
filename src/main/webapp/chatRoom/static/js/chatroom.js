...
function setUserInfo() {
            ...
            async : true,
                success: function(data) {
                    ...
                    if (data.code == 2000) {
                        ...
                        MessageMap = data.customerInfoMap;
                        sentMessageMap = new SentMessageMap();

                        for(var key in data.customerInfoMap){
                            friendList.unshift(data.customerInfoMap[key]);
                            sentMessageMap.put(key, new Array());
                        }
                        console.log(friendList)
                        console.log(data.customerInfoMap)
    ...
    var ws = {
        ...
        registerReceive: function(json) {
            ...
            var friendListHTML = "";
            var friendList = new Array();
            MessageMap = json.visitorInfoMap;
            sentMessageMap = new SentMessageMap();

            for(var key in json.visitorInfoMap){
                friendList.unshift(json.visitorInfoMap[key]);
                sentMessageMap.put(key, new Array());
            }
            console.log(json)
            for (var i = 0; i < friendList.length; i++) {
                ...
                // if(userId != friendList[i].managerId){
                    friendListHTML +=
                        ...
                        '<div class="liLeft"><img src="./static/img/avatar/Member002.jpg"></div>' +
                        '<div class="liRight">' +
                        '<span class="hidden-userId">' + friendList[i].visitorId + '</span>' +
                        '<span class="intername">' + friendList[i].visitorName + '</span>' +
                        '<span class="infor"></span>' +
                        '</div>' +
                        '</li>';
                // }
        ...
        offlineReceive:function(data){
            ...
            var answer='';
            answer += '<li class="time-li"><span style="text-align: center;color: #AAAAAA; padding: .5rem; width: 50%;margin: 0 auto; border-radius: 5px;">[系统消息] 该用户已下线</span></li>';
        ...
        singleReceiveProduct: function(data) {
            // 获取、构造参数
            console.log(data);
            ...
            //
            // }
            messageArray = sentMessageMap.get(fromUserId);
        ...
        showHistory: function(data,historyMap,visitorDto) {
            // 获取、构造参数
            console.log(data);
        ...
        singleFirstReceive: function(data) {
            ...
            answer += '<li>' +
                ...
                '<div class="nesHead"><img src="' + fromAvatarUrl + '"/></div>' +
                '</li>';
    ...
    function showHistoryList(list,userId,toId){
        ...
        html += '<div style="text-align: center;background: rgba(0,0,0,.5);color: #fff; padding: .5rem; width: 50%;margin: 0 auto; border-radius: 5px;">[系统消息] 该用户正在向您咨询，<a onclick="sendOpen('+toId+')">马上回复</a></div>';
        console.log(sentMessageMap);
        console.log(toId);

        sentMessageMap.get(toId).push(html);
